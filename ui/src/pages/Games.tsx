...
import { createEffect, createResource, createSignal, For, Match, onCleanup, Show, Switch as SolidSwitch } from "solid-js";
...
export const Games = () => {
  ...
  const gamesPlaying = (games: Game[]): boolean => {
    return games.filter(game => game.status.toLowerCase() !== "ppd").some(game => game.status.toLowerCase().includes("q") || game.status.toLowerCase().includes("h"));
  };
  ...
  return (
    <main class="flex flex-grow flex-col bg-primary pt-4">
      <SolidSwitch
        fallback={
          <AnimationDiv>
            <div class="text-center text-white">No models found</div>
          </AnimationDiv>
        }
      >
        ...
        <Match when={games.length === 0 && games.state !== "refreshing"}>
          <AnimationDiv class="flex flex-grow items-center justify-center">
            <div class="text-center">
              ...
              <div class="text-lg font-semibold text-white light:text-black">
                <p>
                  Check out the{" "}
                  <a href="https://www.nba.com/schedule" target="_blank" class="pb-1 underline underline-offset-4 hover:text-300">
                    NBA&nbsp;Schedule
                  </a>{" "}
                  to see when the next game is.
                </p>
              </div>
            </div>
          </AnimationDiv>
        ...
        <Match when={!games.error}>
          <AnimationDiv class="container grid grid-cols-1 text-white light:text-black lg:grid-cols-4">
            <div class="col-span-3">
              <div class="mx-2">
                <div class="mb-10 flex flex-row items-center justify-center">
                  <Show when={predictions.loading}>
                    <AnimationDiv class="mr-2 h-4 w-4 animate-spin rounded-full border-b-2 border-white"></AnimationDiv>
                  </Show>
                  <Select
                    options={models()!!}
                    ...
                    value={selectedModel()}
                    itemComponent={props => (
                      <SelectItem class={"border-700 bg-transparent text-center text-white hover:bg-secondary"} item={props.item}>
                        {props.item.rawValue}
                      </SelectItem>
                    )}
                  >
                    <SelectTrigger aria-label="models" class="w-[180px] border-2 border-700 bg-primary text-white">
                      <SelectValue<string>>{state => state.selectedOption()}</SelectValue>
                    </SelectTrigger>
                    <SelectContent class="bg-primary" />
                  </Select>
                </div>
                <AnimationDiv id="options" class="mx-auto mb-3 flex w-full max-w-4xl flex-row items-center justify-between text-white">
                  <div class="flex items-center text-sm">
                    <FiCalendar class="mr-1 inline-block h-4 w-4" />
                    <span class="ml-2">{formattedDateForUser(games()!![0].start_time_unix)}</span>
                  </div>

                  <AnimationDiv class="flex flex-row items-center">
                    <span class="mr-2 text-sm">Live Updates</span>
                    <SimpleTooltip trigger={<Switch checked={liveUpdates()} onChange={setLiveUpdates} disabled={!gamesPlaying(games()!!)} />}>{gamesPlaying(games()!!) ? `Live updates will be gathered every 30 seconds` : `Live updates are disabled until games start`}</SimpleTooltip>
                  </AnimationDiv>
                </AnimationDiv>
