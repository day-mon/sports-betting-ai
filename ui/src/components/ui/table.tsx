...
const TableRow: Component<ComponentProps<'tr'>> = (props) => {
  const [, rest] = splitProps(props, ['class']);
  return <tr class={cn('hover:bg-muted/50 data-[state=selected]:bg-muted border-b transition-colors', props.class)} {...rest} />;
};

const TableHead: Component<ComponentProps<'th'>> = (props) => {
  const [, rest] = splitProps(props, ['class']);
  return <th class={cn('text-muted-foreground h-12 px-4 text-left align-middle font-medium [&:has([role=checkbox])]:pr-0', props.class)} {...rest} />;
};
